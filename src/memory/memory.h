...
public:
  ...
  explicit my_shared_ptr(pointer ptr)
      : _ptr(ptr), _count(new std::atomic<int>(1)) {}
  explicit my_shared_ptr(my_shared_ptr &other)
      : _ptr(other._ptr), _count(other._count) {
    if (_ptr)
      ++(*_count);
  }
  explicit my_shared_ptr(const my_shared_ptr &other)
      : _ptr(other._ptr), _count(other._count) {
    if (_ptr)
      ++(*_count);
  }
...
template <class T> inline my_shared_ptr<T> make_my_shared(T *ptr) {
...
public:
  ...
  explicit my_unique_ptr(pointer ptr) { _ptr = ptr; }
...
template <class T> inline my_unique_ptr<T> make_my_unique(T *ptr) {
