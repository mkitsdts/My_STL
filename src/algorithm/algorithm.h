...
namespace STL
{
	...
	inline static void sort(RandomAccessIterator begin, RandomAccessIterator end, Compare compare)
	{
		auto size = end - begin;
		if (size < 10)
		{
	...
	{
		sort(begin, end, [](RandomAccessIterator begin, RandomAccessIterator end){ return *begin < *end; });
	}

	template <typename RandomAccessIterator, typename Compare>
	static void quick_sort(RandomAccessIterator begin, RandomAccessIterator end, Compare compare)
	{
		if (begin >= end)
		{
			return;
	...
	static void merge_sort(RandomAccessIterator begin, RandomAccessIterator end, Compare compare)
	{
	...
	template <class RandomAccessIterator, class Compare>
	inline static void shell_sort(RandomAccessIterator begin, RandomAccessIterator end, Compare compare)
	{
		auto size = end - begin;
		auto gap = (size + 1) / 2;
		while (gap > 0)
	...
	template <class RandomAccessIterator, class Compare>
	inline static void heap_sort(RandomAccessIterator begin, RandomAccessIterator end, Compare compare)
	{
		auto heapify = [&](RandomAccessIterator begin, int size, int i)
			{
				int largest = i;
				int left = 2 * i + 1;
				int right = 2 * i + 2;
